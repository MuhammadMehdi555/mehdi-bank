...
if(isset($_POST['submit'])){
  ...
  $chack_customer=mysqli_query($connect,"SELECT * FROM customers WHERE account_number='$ac_nu'");
  if(mysqli_num_rows($chack_customer)>0){
    ...
    if($customer['created_by']==$emp_id){
      ...
      if ($pkr > $old_balance) {
    echo "<script>
        alert('Withdrawal failed! Insufficient balance. Available Balance: $old_balance');
    </script>";
}
else{
      if($customer['status']==0){
          echo "<script>
        alert('Withdrawal failed! Your account is closed.');
    </script>";
    }
    else{
      $new_balance=$old_balance-$pkr;
      $update=mysqli_query($connect,"UPDATE customers SET deposit='$new_balance' WHERE account_number='$ac_nu'");
      $insert=mysqli_query($connect,"INSERT INTO transfers (account_id, transfar_type,amount,balance_after,related_account,description,created_by,created_at,created_time) VALUES ('$ac_nu','withdraw','$pkr','$new_balance',NULL,'$remark','$emp_id',NOW(),CURTIME())");
  
    if($insert&&$update){
              echo "<script>alert('Withdraw successful! Old Balance: $old_balance | New Balance: $new_balance');
              window.location.href='transactions-withdraw.php';
              </script>";
    }
    else{
    echo "<script>
...
<head>
  ...
  <title>Withdraw • Staff Banking Panel</title>
  <!-- Google Fonts -->
  ...
  <div class="container-fluid">
    <div class="row flex-nowrap">
      ...
      <main class="col py-4 content-area">
        <div class="container-fluid">
          <h1 class="page-title h3 fw-bold"><i class="fa-solid fa-circle-dot text-primary"></i> Withdraw</h1>

<div class="card">
  <div class="card-header">
    <h2 class="h6 mb-0"><i class="fa-solid fa-circle-arrow-up me-2 text-primary"></i> Withdraw</h2>
  </div>
  <div class="card-body">
    <form method="POST">
      <div class="row g-3">
        <div class="col-md-4">
          ...
          <input type="text" class="form-control" name="ac_nu" placeholder="ac_877138649404" required>
        </div>
        <div class="col-md-4">
        ...
        <button href="transactions-history.php" type="submit" name="submit" class="btn btn-primary">Submit Withdrawal</button>
        </form>
